...
def dataanalyse(df):
    ...
    # 计算每个药 在每家药店卖的次数
    drug = df.groupby([df['pharmacy_id'],df['med_cd']]).size().reset_index()
    ...
    #计算平均价格>200的药
    price = df['exam_post_uprc'].groupby(df['med_cd']).mean().reset_index()


    df_2 = df_1[df_1['med_cd'].isin(price[price['exam_post_uprc']>200]['med_cd'])]
    df_2['ratio'] = df_2['durgSum']/df_2['orderSum']


    df_3 = df_2.pivot(index='pharmacy_id', columns='med_cd', values='ratio')
    ...
    df_4 = df_2.pivot(index='med_cd', columns='pharmacy_id', values='ratio')
    df_4['maxratio'] = df_list_max
    print(df_4[df_4['maxratio']>0.1])
